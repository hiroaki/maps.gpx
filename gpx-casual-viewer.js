...
GPXCasualViewer.GPXToJSON = function( xml_document ) {
  ...
  gpx = gpxTypeToJson( xml_document.getElementsByTagName(GPXCasualViewer.ELEMENTS.GPX)[0] );
...
GPXCasualViewer.prototype.use = function(plugin_name) {
  var hook      = GPXCasualViewer.plugin[plugin_name].hook,
      callback  = GPXCasualViewer.plugin[plugin_name].callback;
  try {
...
GPXCasualViewer.require_plugin = function (plugin_name){
  ...
  return GPXCasualViewer.load_script(src).then((function (){
    ...
    bundles = GPXCasualViewer.plugin[this.plugin_name].bundles || [];
    for ( i = 0, l = bundles.length; i < l; ++i ) {
      ...
      if ( bundles[i].match(/\.css$/) ) {
        load_scripts.push(GPXCasualViewer.load_css(bundle));
      } else {
        load_scripts.push(GPXCasualViewer.load_script(bundle));
      }
    }
